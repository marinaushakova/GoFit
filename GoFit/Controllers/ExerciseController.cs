...
namespace GoFit.Controllers
{
    public class ExerciseController : GoFitBaseController
    {
        ...
        public PartialViewResult ExerciseDetails(int? ex_id)
        {
            ...
            if (ex_id == null)
            {
                return PartialView("_DetailedErrorPartial", new HttpStatusCodeResult(HttpStatusCode.BadRequest, "Exercise could not be retrieved with given parameters."));
            }
            exercise = db.exercises.Find(ex_id);
            if (exercise == null)
            {
                return PartialView("_DetailedErrorPartial", new HttpStatusCodeResult(HttpStatusCode.NotFound, "Could not find the specified exercise."));
            }
